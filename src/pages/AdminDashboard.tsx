...
import { Shield, CheckCircle, AlertTriangle, MessageSquare, BarChart2, Eye, Flag } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-4">
               {[1, 2].map(i => (
                 <div key={i} className="glass-card p-6 flex items-center justify-between group hover:border-brand-gold/30 transition-all">
                    <div className="flex items-center gap-4">
                       <div className="w-12 h-12 rounded-xl bg-brand-brown" />
                       <div>
                          <h4 className="font-bold text-sm">Amara K. (Artiste Draft)</h4>
                          <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Oeuvre : 'Esprit de Savane' • 12K vues</p>
                       </div>
                    </div>
                    <div className="flex gap-2">
                       <button className="px-4 py-2 bg-brand-gold text-brand-black text-[10px] font-black rounded-lg">APPROUVER</button>
